...
@dag(
    dag_id='economy_sentiment_etl_dag',
    default_args=default_args,
    description='경제 심리 지수',
    schedule_interval=timedelta(days=1),
    start_date=datetime(2023, 1, 1),
    catchup=False
)
def economy_sentiment_etl():

    @task(task_id="fetch_data")
    def fetch_data():
        end_date = datetime.now().strftime('%Y%m')

        url = f"https://ecos.bok.or.kr/api/StatisticSearch/GZJ2WT8Y559OMJKLPMRQ/json/kr/1/99999/513Y001/M/199001/{end_date}"
    ...
    def upload_to_s3(file_path: str):
        ...
        s3_key = 'newb_data/bank_of_korea/raw_data/EconomySentiment.csv'
