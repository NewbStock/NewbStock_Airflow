...
def update_stock_data():
    ...
    for _, company in kr_top100.iterrows():
        ...
        try:
            ...
            df['Date'] = df['Date'].astype(str)  # Avro requires string for date
...
with DAG ( 
    dag_id='get_kr_stock_data_avro',
    default_args=default_args,
    description='Save file as Avro',
    schedule_interval='30 9 * * 1-5',  # UTC 09:30 (KST 18:30), 월요일부터 금요일까지 장 마감 후  
    start_date=datetime(2024, 7, 20),  # 시작 날짜
    catchup=False,
) as dag:

    process_and_upload_stock_data_task = PythonOperator(
        task_id='save_stock_data_as_avro',
